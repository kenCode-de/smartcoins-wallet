...
/**
 * Represents a Dash coin Account.
 *
...
public class DashAccount extends GeneralCoinAccount {
    ...
    /**
     * Network parameters for the Dash coin, it's a custrom network param used in bitcoinj library
     */
    private NetworkParameters mParam = CustomNetworkParameters.fromCoin(DASH);

    /**
     * The account number for Dash coin, as the SLIP-44
     */
    private static final int sDashCoinNumber = 5;
    ...
    DashAccount(long id, String name, AccountSeed seed, int accountNumber, int lastExternalIndex, int lastChangeIndex) {
        super(id, name, DASH, seed, sDashCoinNumber, accountNumber, lastExternalIndex, lastChangeIndex);
    }
    ...
    DashAccount(final AccountSeed seed, String name, boolean importing) {
        super(-1, name, DASH, seed, sDashCoinNumber, 0, 0, 0);
        if (importing) {
     ...
     *
     * @return This list only contains one balance, that is for the Dash coin
     */
    @Override
    ...
    /**
     * Creates a transaction and broadcast it to the Dash coin network
     *
